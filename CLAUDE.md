...
- **Тикеты и роадмап:** только в GitHub project — в репо файлов с фазами/тикетами нет, не создавай их.
...
- **Python (FastAPI + pika + uv):** `services/auth` (`src/main.py:1`), `services/transcoder` (`app/consumer.py:1`) — `python:3.14-slim`, `uv`, `argon2`, `aiobotocore`, `ffmpeg-python`.
...
Локально без Docker: `make dev-auth` (`:8001`), `make dev-transcoder` (pika-воркер), `make dev-metadata` (`:8002`), `make dev-upload` (`:8003`), `make dev-gateway` (`:8080`), `make dev-frontend` (`:3000`).
